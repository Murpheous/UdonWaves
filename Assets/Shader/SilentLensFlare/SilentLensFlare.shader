Shader "Silent/Sunshine Flare"
{
    Properties
    {
        ...
        [Space]
        _VisRadius("Visibility Range", Range(0.01, 1.0)) = 1.0
        [Space]
        _Exposure("Exposure", Float) = 1.0
        [Toggle(_SUNDISK_HIGH_QUALITY)]_SunDisk("High quality sun rays", Float) = 0
        [Toggle(_SUNDISK_SIMPLE)]_HalfRes("Half-resolution optimization", Float) = 0
        [Space]
...
float3 lensflare(float2 uv, float2 pos, out float sunAura)
{
    ...
    float f0_time = (_Time.xy);
    f0 = f0 + f0*(sin(noise(sin(ang*2.+pos.x+f0_time)*4.0 - cos(ang*3.+pos.y+f0_time))*16.)*.1 + dist*.1 + .8);
...
fixed4 frag (v2f i) : SV_Target
{
    ...
    // This factor dampens the rays when looking away.
    atten = saturate((atten-(1-_VisRadius))/_VisRadius);
    atten *= _RayBrightness;

    float sunRaysRaw = 0;
    
    #if defined(_SUNDISK_SIMPLE)
    if (atten > 0 && QuadGetLaneID() == 0)
    {
        float2 noisePos = (projPos.xy / projPos.w)*(_ScreenParams / 2.0);
        sunRaysRaw = sunRayMask(projPos, lightScr, noisePos);
    };
    sunRaysRaw = QuadReadLaneAt(sunRaysRaw, uint2(0, 0));
    #else
    if (atten > 0)
    {
        float2 noisePos = (projPos.xy / projPos.w)*(_ScreenParams);
        sunRaysRaw = sunRayMask(projPos, lightScr, noisePos);
    }
    #endif

    float3 sunRays = sunRaysRaw * lightColor * sunAura * atten;

    // Ensure there isn't a harsh cutoff either
    color = sunGlare + sunRays;
